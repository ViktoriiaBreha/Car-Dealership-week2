...
public class ContractFileManager {
    ...
    public void saveContract (Contract contract){
        try {
            FileWriter fileWriter = new FileWriter("src/main/resources/WorkshopFiles/contracts.csv", true);
            BufferedWriter bufferedWriter = new BufferedWriter(fileWriter);

            if (contract instanceof SalesContract){
                bufferedWriter.write(String.format("Sale|%s|%s|%s|%d|%d|%s|%s|%s|%s|%d|%.2f|%.2f|%.2f|%.2f|%.2f|%" +
                        ".2f|%s|%.2f", contract.date_of_contract, contract.getCustomer_name(),
